...
- **정의**: 기존 코드의 패턴·규격·인프라를 프로젝트 전반에 걸쳐 일괄 변경하는 작업. 2개 이상의 모듈(scope)에 걸쳐 기존 코드를 수정하는 경우 해당
...
- **식별**: 전체 영향 작업 PR에는 `global-impact` 라벨 부착. 작업 시작 전 해당 라벨의 열린 PR 존재 여부 확인
- **이유**: 전체 영향 작업은 다수 파일을 수정하므로, 다른 작업이 먼저 병합되면 rebase 충돌이 폭발적 증가. 반대 순서면 독립 작업의 충돌 범위는 관리 가능

### 원칙 2: 독립 작업 격리 (Worktree Isolation)

전체 영향 작업이 아닌 모든 작업은 **독립적 단위로 구성**하고 **별도 worktree에서 처리**한다.

- **작업 시작 전 충돌 확인**: 열린 PR, 진행 중 기능 작업과의 파일 겹침 검토
- **worktree 필수**: 모든 독립 작업은 feature branch를 별도 worktree에서 체크아웃하여 진행
