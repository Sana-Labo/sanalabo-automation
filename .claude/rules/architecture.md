...
[LINE Bot (단일 채널)] ←──→ [Webhook + Event Router]
    ...
    (per-workspace                  [Member 0] [Member 1] [Member 2]
     OAuth token)                   least-inflight dispatch
           │                               │
    [googleapis (in-process)]       [LINE Push API]
```
...
| **Native Tool** | 에이전트가 직접 실행하는 도구 | googleapis (in-process) |
...
|-------|----------|-------|------|
| Google Workspace | Native Tool (`googleapis` in-process) | `gmail_*`, `calendar_*`, `drive_*` | 워크스페이스별 OAuth 토큰으로 격리 |
...
Webhook event
  ...
  → deps.getGwsExecutors(workspace.id) — TokenStore → OAuth2Client → API executor
