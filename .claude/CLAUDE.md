...
- **대상**: 로그 시스템 도입, 전체 주석 규격 변경, 아키텍처 리팩토링, 신기술 전면 적용 등
- **병합 순서**: 전체 영향 작업의 PR이 main에 병합될 때까지 다른 작업의 PR 병합 보류
- **개발은 자유**: 독립 작업의 개발 자체는 별도 worktree에서 병렬 진행 가능
- **이유**: 전체 영향 작업은 다수 파일을 수정하므로, 다른 작업이 먼저 병합되면
  rebase 충돌이 폭발적 증가. 반대 순서면 독립 작업의 충돌 범위는 관리 가능
...
- **worktree 필수**: 모든 독립 작업은 EnterWorktree로 분리된 세션에서 진행
