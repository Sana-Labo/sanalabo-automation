...
Claude API の tool_use を活用して Google Workspace を操作するエージェントサーバー。
LINE を入力チャネルとして使用し、ワークスペース単位のマルチテナントアーキテクチャで動作する。
...
| Term | Description |
|------|-------------|
| **Workspace** | GWS アカウント + メンバーグループの単位 |
| **Owner** | ワークスペースの所有者。GWS の読み書き権限を持つ |
| **Member** | ワークスペースのメンバー。読み取りは自由、書き込みは Owner 承認が必要 |
| **System Admin** | システム全体の管理者。ワークスペースのプロビジョニングを行う |

### Agent Core

Intent router ではなく **エージェントループ** — Claude がどのツールを呼び出すか自律的に判断する。

- **Google Workspace**: Native Tool (`Bun.spawn` → GWS CLI)
- **LINE メッセージ**: MCP Tool (`@line/line-bot-mcp-server` 経由)

Member の書き込み操作は Owner の承認フロー（PendingAction）を経由する。
...
| AI | `@anthropic-ai/sdk` — tool_use ベースのエージェントループ |
| GWS | `@googleworkspace/cli` — Native Tool (`Bun.spawn`) |
| LINE | `@line/line-bot-mcp-server` — MCP Tool |
| MCP Transport | Connection Pool (N 個の stdio プロセス, least-inflight dispatch) |
